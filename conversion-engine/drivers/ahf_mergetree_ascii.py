    ...
    with open(filepath) as fh:
        for line in fh:
            ...
            try:
                ...
                print(f"WARNING: skipping malformed line in {filepath}: {exc}",
                      file=sys.stderr)
                continue
            halos.append(h)
    return halos
...
def read_trees(
    ...
    def _tree_sort_key(root):
        ...
        snap_max_m = [halo_props[h]['Mhalo'] for h in members
    ...
    for tree_idx, root in enumerate(tqdm(sorted_roots[:n_trees_read], desc="Reading trees")):
        ...
        for flat_idx, hid in enumerate(ordered_hids):
            ...
            valid = [(m, pid) for m, pid in prog_list
...
def convert(
    ...
    try:
        ...
        # ------------------------------------------------------------------
        input_dir = Path(input_path)
        halos_files = sorted(glob(str(input_dir / "*.AHF_halos")),
                             key=_snap_from_filename)
        croco_files = sorted(glob(str(input_dir / "*.AHF_croco")),
                             key=_snap_from_filename)
        ...
        print(f"  AHF_halos files : {n_snap}  (snap {min_snap}–{max_snap})",
              file=sys.stderr)
        ...
        if particle_mass is None:
            top_hids = sorted(snap_halos[max_snap],
        ...
        print(f"  Particle mass : {m_particle_msun:.4e} Msun/h "
              f"({particle_mass_1e10:.6g} × 10^10 Msun/h)", file=sys.stderr)
        ...
        sorted_roots = sorted(tree_members.keys(),
                              key=_tree_sort_key, reverse=True)

        n_trees_total   = len(sorted_roots)
        n_trees_convert = min(n_trees, n_trees_total) if n_trees is not None \
                          else n_trees_total
        print(f"  Trees total / converting: {n_trees_total} / {n_trees_convert}",
              file=sys.stderr)
        ...
        for tree_idx, root in enumerate(sorted_roots[:n_trees_convert]):
            members = tree_members[root]
            ordered = sorted(members,
                             key=lambda h: (-halo_props[h]['snap_id'],
                                            -halo_props[h]['Mhalo']))
            tree_halo_order[root] = ordered
            for flat_idx, hid in enumerate(ordered):
                haloID_to_flat[hid] = (tree_idx, flat_idx)
        ...
        for tree_idx, root in enumerate(tqdm(sorted_roots[:n_trees_convert],
            ...
            for flat_idx, hid in enumerate(ordered_hids):
                ...
                if j_sq > 0.0 and h['lambda'] > 0.0:
                    ...
                    j_mag = 0.0
                subhalo_spin[flat_idx] = [j_mag * h['Lx'],
                                          j_mag * h['Ly'],
                                          j_mag * h['Lz']]
            ...
            for flat_idx, hid in enumerate(ordered_hids):
                h = halo_props[hid]
                central = get_top_central(hid)
                fof_groups[(h['snap_id'], central)].append(
                    (h['Mhalo'], flat_idx)
                )
        ...
        print(f"  Writing {n_trees_convert} trees, {total_halos} halos → {output_path}",
              file=sys.stderr)
