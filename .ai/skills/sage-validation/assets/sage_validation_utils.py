...
def calculate_relative_difference(input_data, output_data, threshold=1e-8, allow_identical=False):
    """
    ...
    if inp.shape == out.shape and np.allclose(inp, out, atol=threshold):
        if not allow_identical:
            raise ValueError("ANTI-CHEATING FAILURE: Input and Output arrays are perfectly identical (rel_diff=0.0). "
                             "If this is expected (e.g., Direct Map with no unit conversions), "
                             "you MUST pass `allow_identical=True` to the plotting functions instead of adding artificial noise. "
                             "Otherwise, verify you are not deriving 'Input' from 'Output'.")
...
def plot_3x3_evolution(plot_id, dataset_name, snapnums, input_arrays, output_arrays, labels, y_label, halo_ids, is_log_y=False, allow_identical=False):
            ...
            if np.array_equal(inp_data[h_idx], out_data[h_idx]):
                if not allow_identical:
                    raise ValueError(f"ANTI-CHEATING FAILURE: Evolution branch for halo {halo_ids[row][h_idx]} "
                                     f"is identical in Input and Output. If expected, pass `allow_identical=True`. "
                                     f"You MUST NOT derive 'Input' from 'Output'.")
...
def plot_1x3_histogram(plot_id, dataset_name, input_data, output_data, x_label, is_log_x=False, is_log_y=False, bins=50, allow_identical=False):
    ...
    rel_diff = calculate_relative_difference(inp_counts, out_counts, allow_identical=allow_identical)
...
def plot_1x3_hexbin(plot_id, dataset_name, inp_x, inp_y, out_x, out_y, x_label, y_label, allow_identical=False):
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    fig.suptitle(f"{plot_id} - {dataset_name}", fontsize=16)
    ...
    diff_r = calculate_relative_difference(inp_r, out_r, allow_identical=allow_identical)
