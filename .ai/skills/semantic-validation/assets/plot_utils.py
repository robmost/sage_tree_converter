...
def save_figure(fig, output_path: str, dpi: int = 150) -> None:
    ...
    dpi : int
        Resolution in dots per inch. Default 150. Use 300 for print-quality.
    """
...
def make_3x3_figure(row_labels=("Top 5", "Median 5", "Bottom 5"),
                    col_titles=("Input", "Output", "Relative difference")):
    """Create a 3×3 figure with shared x axes per row and labelled columns/rows.
...
def rel_diff(output, input_, fill_nan_where_zero=True):
    ...
    Where input == 0, the result is NaN (not 0 or inf).
