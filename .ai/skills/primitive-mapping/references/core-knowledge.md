...
| **Merger Tool** | `Pointer` (SubLink, LHaloTree) | Pre-linked temporal pathways (uses discrete IDs for `FirstProgenitor`, `Descendant`). **CRITICAL WARNING:** Always verify if pointers are globally absolute or **Tree-Local**. If Tree-Local (like L-HaloTree), you MUST partition your validation arrays tree-by-tree to avoid cross-tree jumps. Never assume simulation IDs (like `SubhaloIndex`) are globally unique unless explicitly stated. | **Pointer Parity Map:** Offset incoming local struct IDs or pointers into SAGE global relative pointers. |
