...
Read `.ai/skills/auditor/references/auditor_checklist.md` in full. For each of the 12 items, produce
...
3. A one-line overall verdict at the bottom:
   - `Overall: PASS — all 12 items passed.`
...
If the main agent fixes a failure and asks the auditor to re-check, the auditor must re-read the affected files directly and re-run only the failed items. A full re-audit of all 12 items is required if more than 3 items failed in the previous round.
