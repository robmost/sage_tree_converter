...
| 4 | Velocity distribution uses SubhaloVel modulus | velocity_dist.pdf plots the magnitude `sqrt(Vx² + Vy² + Vz²)` of `SubhaloVel`. It does not plot `SubhaloVMax` or any component of `SubhaloVel` individually. |
| 5 | Angular momentum uses SubhaloSpin vector magnitude | angular_momentum.pdf plots `\|SubhaloSpin\| = sqrt(Jx² + Jy² + Jz²)`. It does not plot the dimensionless spin parameter λ or any scalar spin field. |
