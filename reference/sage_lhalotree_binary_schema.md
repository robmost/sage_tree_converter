...
Offset 8 + 4xnforests  : halo_data[]               - flat array of 104-byte halo records
```
...
| 7 | M_Mean200 | float32 | `f` | 24 | 10^10 Msun/h | No | 0.0 if absent (`Group_M_Mean200`) |
| 8 | M_Crit200 | float32 | `f` | 28 | 10^10 Msun/h | Yes | Mapped from `Group_M_Crit200` (virial/M200c) |
| 9 | M_TopHat | float32 | `f` | 32 | 10^10 Msun/h | No | 0.0 if absent (`Group_M_TopHat200`) |
...
This differs from the HDF5 reader, which applies x0.001 after reading.
...
- Halo load: line 261 (single `pread()` of `nhalos x 104` bytes)
