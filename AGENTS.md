...
3. **Budgeted Discovery Protocol:** When identifying simulation formats, you MUST never perform a monolithic read (e.g., `read()`, `readlines()`) on an unknown file. Always use incremental reads (e.g., `f.readline()` in a loop or `f.read(chunk_size)`). Stop once the format is identified. If searching a directory, check cheap signals (filenames, directory structure) before peeking inside files.
